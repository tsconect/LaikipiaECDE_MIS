...
<?php
  $location = App\Models\Location::where('id', $application->student->location)->first();
?>
...
<div class="card">
    ...
    <h2 class="card-title mb-3">Student Details</h2>
        ...
        @if($application->committeeReviews)
       
        <h2 class="card-title mb-3 mt-3">CDF Committee Review</h2>
        <div class="row">
            <div class="col-sm-3">
                <h6 class="mb-0">Was The Form Dully filled and signed</h6>
            </div>
            <div class="col-sm-9 text-secondary">
                {{$application->committeeReviews->dully_filled_and_signed ?? 'Not Reviewed'}}               
        ...
        <div class="row">
            <div class="col-sm-3">
                <h6 class="mb-0">Was all the documents attached</h6>
            </div>
            <div class="col-sm-9 text-secondary">
                {{$application->committeeReviews->supportive_documents_attached ?? 'Not Reviewed'}}               
            </div>
        </div>
        <hr>
        <div class="row">
        ...
        <div class="row">
            <div class="col-sm-3">
                <h6 class="mb-0">Approval Percentage</h6>
            </div>
            <div class="col-sm-9 text-secondary">
                {{$application->committeeReviews->percentage ?? 'Not Reviewed'}} %
            </div>
        </div>
        <hr>
        <div class="row">
            ...
            <div class="col-sm-9 text-secondary">
                {{$application->committeeReviews->remark ?? 'Not Reviewed'}} 
            </div>
        </div>
        @else
    
            @if($application->chiefsReviews)
            <h4>NGCDF Bursary Committee</h4>
            ...
            <form action="{{ route('admin.reviews.store') }}" method="post">
                ...
                <div class="form-group">
                    <label for="dully_filled_and_signed">Was The Form Dully Filled and Signed</label>
                    <select name="dully_filled_and_signed" id="dully_filled_and_signed" class="form-control" required>
                ...
                <div class="form-group">
                    <label for="supportive_documents_attached">Were All Supportive Documents Attached</label>
                    <select name="supportive_documents_attached" id="supportive_documents_attached" class="form-control" required>
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <p class="text-muted">In a scale of 0 - 100, give a percentage of recommendation for this application</p>
                ...
                <input type="hidden" name="user_id" value="{{ Auth::user()->id ?? '' }}" id="chiefs_id" class="form-control" required>
                <input type="hidden" name="application_id" value="{{ $application->id }}" id="application_id" class="form-control" required>

                <button type="submit" class="btn btn-primary">Submit</button>
            </form>
            @else
                <h4>NGCDF Bursary Committee Review</h4>
                <p > This appications needs to be reviewed by the chief first. Contact chief to review the application <br>
                Name: {{$location->chief->user->first_name??'No chief assigned for this location'}} {{$location->chief->user->last_name??''}} <br>
                Phone Number: {{$location->chief->user->phone_number??'Null'}} <br>
             </p>
            @endif
        @endif
