@extends('admin.app')

@section('nav-bar')
@include('admin.layouts.sidebar')
@endsection

@section('content')
@include('flash-message')

<div class="table-card">
    <div class="table-banner">
        <div class="table-banner-title"><span>VTC</span> DEPARTMENTS</div>
        <div class="banner-actions">
            <a href="{{ route('admin.const.create') }}">
                <button class="btn-new" type="button">New Department</button>
            </a>
        </div>
    </div>

    <table id="example" class="data-table dt-admin">
        <thead>
            <tr>
                <th>ID</th>
                <th>NAME</th>
                <th>CAPACITY</th>
                <th>DATE CREATED</th>
                <th>DATE UPDATED</th>
                <th>ACTION</th>
            </tr>
        </thead>
        <tbody>
            @foreach($data as $item)
                <tr>
                    <td class="td-id">{{ $item->id }}</td>
                    <td>{{ $item->department_name }}</td>
                    <td>{{ $item->capacity }}</td>
                    <td>{{ $item->created_at }}</td>
                    <td>{{ $item->updated_at }}</td>
                    <td>
                        <div class="action-btns">
                            <a class="act-btn view" title="Add Courses" href="{{ url('admin/new-vtc_courses?dpt=' . $item->id) }}"><i class="fa fa-plus"></i></a>
                            <a class="act-btn edit" title="Edit" href="{{ url('admin/constituency/edit/' . $item->id) }}"><i class="fa fa-edit"></i></a>
                            <a class="act-btn delete" title="Delete" href="{{ route('admin.delete-constituency', $item->id) }}"><i class="fa fa-trash"></i></a>
                        </div>
                    </td>
                </tr>
            @endforeach
        </tbody>
    </table>

</div>
@endsection