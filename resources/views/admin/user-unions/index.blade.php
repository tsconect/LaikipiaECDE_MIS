...
@section('content')
    ...
    <div class="table-card">
        ...
        <div class="table-banner">
            <div class="table-banner-title"><span>USER</span> UNIONS</div>
            <div class="banner-actions">
                <a href="{{ route('admin.user-unions.create') }}">
                    <button class="btn-new">
                        <svg viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd"/></svg>
                        Add Union Membership
                    </button>
                </a>
            </div>
        </div>
        ...
        <!-- Table -->
        <table class="data-table" id="userUnionsTable">
            <thead>
                <tr>
                    ...
                    <th>MEMBERSHIP NUMBER <span class="sort-arrows"><svg viewBox="0 0 8 8" fill="currentColor"><path d="M4 0l2 3H2L4 0z"/></svg><svg viewBox="0 0 8 8" fill="currentColor"><path d="M4 8L2 5h4L4 8z"/></svg></span></th>
                    <th>ACTION</th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <a class="act-btn view" title="View Union Membership" href="{{ route('admin.user-unions.show', $item->id) }}">
                                    <svg viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z"/><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd"/></svg>
                                </a>
                                <a class="act-btn edit" title="Edit Union Membership" href="{{ route('admin.user-unions.edit', $item->id) }}">
                                    <svg viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
                                </a>
                                <form action="{{ route('admin.user-unions.destroy', $item->id) }}" method="POST" class="inline-form" onsubmit="return confirm('Delete this union membership?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        ...
        function copyTable() {
            const table = document.getElementById('userUnionsTable');
        ...
        function exportCSV() {
            ...
            for (let row of table.rows) {
                ...
                for (let cell of row.cells) {
                    if (cell.cellIndex < row.cells.length - 1) { // Skip action column
        ...
        function printTable() {
            ...
            printWindow.document.write('<html><head><title>User Unions</title></head><body>');
            printWindow.document.write('<h2>User Unions</h2>');
        ...
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            const rows = document.querySelectorAll('#userUnionsTable tbody tr');
            rows.forEach(row => {
